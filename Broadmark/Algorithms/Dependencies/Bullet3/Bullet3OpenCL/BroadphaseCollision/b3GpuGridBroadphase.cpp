...
#define B3_GRID_BROADPHASE_PATH "Dependencies/Bullet3OpenCL/BroadphaseCollision/kernels/gridBroadphase.cl"
...
// _gridbp pra evitar qualquer problema de linker dps.
B3_FORCE_INLINE bool b3TestAabbAgainstAabb2_gridbp(const b3Vector3 &aabbMin1, const b3Vector3 &aabbMax1,
...
void  b3GpuGridBroadphase::calculateOverlappingPairsHost(int maxPairs)
{
	...
	for (int i=0;i<m_allAabbsCPU1.size();i++)
	{
		...
		{
			if (b3TestAabbAgainstAabb2_gridbp(m_allAabbsCPU1[i].m_minVec, m_allAabbsCPU1[i].m_maxVec,
